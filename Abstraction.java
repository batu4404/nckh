...
//		}
	...
	public List<String> formularize(CtBlock block, List<Variable> listVariables) {
		...
		for(CtStatement s: statements) {
			statement = formularize(s, listVariables);
			if(statement != null) {
				if(f == null)
					f = statement;
				else
	...
	public List<String> formularize(CtStatement statement, List<Variable> listVariables) {
		...
		if(statement instanceof CtAssignment) {
			...
			s = formularize( (CtLocalVariable) statement, 
												listVariables);
		}
	...
	public List<String> formularize(CtFor loop, List<Variable> listVariables) {
		...
		for(CtStatement s: forInit) {
			fStatement = wrapAll( formularize(s, listVariables), "and");
			if(fStatement != null)
				f.add(fStatement);
		}
		...
		List<CtStatement> listStatements = new ArrayList<>();
		listStatements.add(body);
		listStatements.addAll(forUpdate);
		CtBlock block = new CtBlockImpl();
		block.setStatements(listStatements);
											...
											List<Variable> listVariables) 
	{
		...
		if(initializerExp != null) {
			v.initialize();
			String initializer = formularize(initializerExp, listVariables);
			f = wrap(v.getValue(), "=", initializer); 
		}
	...
	private String updateAVar(List<Variable> l1, List<Variable> l2, String varName) {
		Variable v1 = Variable.getVariable(varName, l2);
